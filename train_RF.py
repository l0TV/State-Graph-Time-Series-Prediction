...
from dataloader import get_ml_loader, get_feature_analysis_loader
import scipy.io as io


def single_rf_train(is_train=True):
    set_seed(1022)
    save_path = f'./experiment/main_exp/RF/'
    train_x, train_y, test_x, test_y, mmn = get_feature_analysis_loader()

    if is_train:
        estimator = RandomForestRegressor(n_estimators=30, max_depth=8, random_state=1022)
        estimator.fit(train_x, train_y)
        # 保存实验结果
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        joblib.dump(estimator, save_path + 'rf.pkl')

    rf = joblib.load(save_path + 'rf.pkl')
    kinds = os.listdir('./turned_dataset/LTE_Dataset')
    f = open(save_path + 'exp.txt', 'w+')
    for kind in kinds:
        f.write('-------' + kind + '-------\n')
        y_pred = mmn.inverse_transform(rf.predict(test_x[kind]), -1)
